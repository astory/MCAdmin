...
namespace MCAdmin
{
    public class MCFirewall
    {
        ...
        public void EnforceBans()
        {
            ...
            {
                if ((!ply.IsDev()) && (Program.bannedIPs.Contains(ply.ip) || ply.GetRank() == "banned" || (Program.mbansEnable && Program.masterBanList.Contains(ply.name.ToLower())))) ply.Disconnect("You're banned!");
            }
        }
